...
public class StartingClass extends JPanel implements Runnable, MouseListener, MouseMotionListener, KeyListener {
	...
	public static boolean level_select, started, clicked_ip_box, player2, player3, player4, to_start, // started => main
																										// game started;
																										// clicked_ip_box
																										// => ipbox
																										// clicked;
	...
	public void start() {
		...
		paddle = new Paddle(80, border_right / 2, border_right - barrier_height, barrier_height, 20, -1);
		comp_paddle = new Paddle(80, border_right / 2, border_right - barrier_height, barrier_height, 20, -1);
		left_paddle = new Paddle(80, border_right / 2, border_right - barrier_height, barrier_height, 20, -1);
		right_paddle = new Paddle(80, border_right / 2, border_right - barrier_height, barrier_height, 20, -1);
		int radius = 10;
		for (int b = 0; b < n_balls; b++)
			ball[b] = new Ball(paddle.getPos() + (2 * paddle.getSize() * (2 * b - n_balls)) / (5 * n_balls),
					border_bottom - paddle.height - radius, radius);
		gameNotStarted();
	...
	public void paint1(Graphics g) {
		...
		if (paddle_life_rec > 0)
			g.drawImage(Paddle, paddle.getPos() - paddle.getSize() / 2, border_bottom - paddle.height, paddle.getSize(),
					paddle.height, this);
		if (comp_life_rec > 0)
			g.drawImage(Paddle, border_right - disp_comp - comp_paddle.getSize() / 2, border_top, comp_paddle.getSize(),
					comp_paddle.height, this);
		if (left_life_rec > 0)
			g.drawImage(Paddle, border_left, disp_left - left_paddle.getSize() / 2, left_paddle.height,
					left_paddle.getSize(), this);
		if (right_life_rec > 0)
			g.drawImage(Paddle, border_right - right_paddle.height,
					border_bottom - disp_right - right_paddle.getSize() / 2, right_paddle.height,
					right_paddle.getSize(), this);
		// barriers
		g.drawImage(Paddle, border_left, border_top, barrier_height, barrier_height, this);
		g.drawImage(Paddle, border_right - barrier_height, border_top, barrier_height, barrier_height, this);
		g.drawImage(Paddle, border_left, border_bottom - barrier_height, barrier_height, barrier_height, this);
		g.drawImage(Paddle, border_right - barrier_height, border_bottom - barrier_height, barrier_height,
				barrier_height, this);
		...
		case 5:
			g.drawImage(Life, border_right + 105, border_top + 120, 15, 15, this);
		case 4:
			g.drawImage(Life, border_right + 125, border_top + 120, 15, 15, this);
		case 3:
			g.drawImage(Life, border_right + 145, border_top + 120, 15, 15, this);
		case 2:
			g.drawImage(Life, border_right + 165, border_top + 120, 15, 15, this);
		case 1:
			g.drawImage(Life, border_right + 185, border_top + 120, 15, 15, this);
			break;
		case 0:
			g.setColor(Color.WHITE);
			g.setFont(new Font("TimesRoman", Font.BOLD, 15));
		...
		case 5:
			g.drawImage(Life, border_right + 105, border_top + 160, 15, 15, this);
		case 4:
			g.drawImage(Life, border_right + 125, border_top + 160, 15, 15, this);
		case 3:
			g.drawImage(Life, border_right + 145, border_top + 160, 15, 15, this);
		case 2:
			g.drawImage(Life, border_right + 165, border_top + 160, 15, 15, this);
		case 1:
			g.drawImage(Life, border_right + 185, border_top + 160, 15, 15, this);
			break;
		case 0:
			g.setColor(Color.WHITE);
			g.setFont(new Font("TimesRoman", Font.BOLD, 15));
	...
	@Override
	public void run() {
		float x, y, radius = ball[0].getSize(), paddle_pos, paddle_size, comp_paddle_pos, comp_paddle_size,
				right_paddle_pos, right_paddle_size, left_paddle_size, left_paddle_pos;
		boolean flag = true;
		...
		while (true) {
			...
			for (int b = 0; b < n_balls; b++) {
				...
				if ((border_top + barrier_height >= y - radius || border_bottom - barrier_height <= y + radius)
						&& x - radius <= border_left + barrier_height
						&& x - radius - ball[b].getSpeedX() > border_left + barrier_height) {
					ball[b].setSpeedX(Math.abs(ball[b].getSpeedX()));
				...
				if ((border_top + barrier_height >= y - radius || border_bottom - barrier_height <= y + radius)
						...
						&& x + radius - ball[b].getSpeedX() < border_right - barrier_height) {
					ball[b].setSpeedX(-Math.abs(ball[b].getSpeedX()));
				...
				if ((x - radius <= border_left + barrier_height || x + radius >= border_right - barrier_height)
						&& border_top + barrier_height >= y - radius
						&& border_top + barrier_height < y - radius - ball[b].getSpeedY()) {
					ball[b].setSpeedY(Math.abs(ball[b].getSpeedY()));
				...
				if ((x - radius < border_left + barrier_height || x + radius > border_right - barrier_height)
						...
						&& border_bottom - barrier_height > y + radius - ball[b].getSpeedY()) {
					ball[b].setSpeedY(-Math.abs(ball[b].getSpeedY()));
				...
				// ball[b].isOnPaddle()+" "+ball[b].getSpeedX()+" "+ball[b].getSpeedY()+"
				// "+(ball[b].getSpeedX()==0));
				if (ball[b].isOnPaddle()) {
					// ball[b].setOnPaddle(false);
					switch (playerNum) {
					case 1:
						ball[b].setX(paddle.getPos() + (2 * paddle.height * (2 * b - n_balls)) / (5 * n_balls));
						ball[b].setY(border_bottom - paddle.height - ball[b].getSize());
						break;

					case 2:
						ball[b].setY(border_bottom - (right_paddle.getPos()
								+ (2 * right_paddle.height * (2 * b - n_balls)) / (5 * n_balls)));
						ball[b].setX(border_right - right_paddle.height - ball[b].getSize());
						break;

					case 3:
						ball[b].setX(border_right - (comp_paddle.getPos()
								+ (2 * comp_paddle.height * (2 * b - n_balls)) / (5 * n_balls)));
						ball[b].setY(comp_paddle.height + ball[b].getSize());
						break;

					case 4:
						ball[b].setY(
								left_paddle.getPos() + (2 * left_paddle.height * (2 * b - n_balls)) / (5 * n_balls));
						ball[b].setX(left_paddle.height + ball[b].getSize());
						break;
					}
					// System.out.println("FEWFWEFWEf___ "+playerNum+" "+ball[b].getX()+"
					// "+ball[b].getY());
				}
			...
			if (broadcastBall) {
				try {
					...
					if (!(paddle_life == paddle_life_old && comp_life == comp_life_old && left_life == left_life_old
							&& right_life == right_life_old))
	...
	public void mouseClicked(MouseEvent arg0) {
		...
		if (to_start && !level_select && !started && arg0.getX() > 80 && arg0.getX() < 340 && arg0.getY() > 100
				&& arg0.getY() < 130) {
			clicked_ip_box = true;
			ip_text = "";
		} else if (to_start && create_not_join && !level_select && !started && arg0.getX() > 80 && arg0.getX() < 400
			...
			player4 = !player4;
				...
				&& (arg0.getX() > 100 && arg0.getX() < 340 && arg0.getY() > 60 && arg0.getY() < 90)) {
			level = 0;
			...
			PlayGame.startGettingData(this);
				...
				&& (arg0.getX() > 100 && arg0.getX() < 340 && arg0.getY() > 100 && arg0.getY() < 130)) {
			System.out.println("FWEEFWEFWWEWEFWFWEFWE  " + level);
			level = 1;
			started = true;
			clicked_ip_box = false;
			...
			PlayGame.startGettingData(this);
				...
				&& (arg0.getX() > 100 && arg0.getX() < 340 && arg0.getY() > 140 && arg0.getY() < 170)) {
			System.out.println("FWEEFWEFWWEWEFWFWEFWE  " + level);
			level = 2;
			started = true;
			clicked_ip_box = false;
			...
			PlayGame.startGettingData(this);
				...
				&& !(arg0.getX() > 80 && arg0.getX() < 400 && arg0.getY() > 80 && arg0.getY() < 280)) {
			level_select = true;
			clicked_ip_box = false;
		} else if (to_start && !create_not_join && !started
				&& !(arg0.getX() > 80 && arg0.getX() < 400 && arg0.getY() > 80 && arg0.getY() < 280)) {
			started = true;
			clicked_ip_box = false;
			...
			allJoined = true;
			PlayGame.startGettingData(this);
		} else if (!to_start && (arg0.getX() > 100 && arg0.getX() < 340 && arg0.getY() > 100 && arg0.getY() < 130)) {
			to_start = true;
			create_not_join = true;
		} else if (!to_start && (arg0.getX() > 100 && arg0.getX() < 340 && arg0.getY() > 140 && arg0.getY() < 170)) {
			to_start = true;
			create_not_join = false;
		}
	}
	...
	public void mousePressed(MouseEvent arg0) {
		...
		if (started && ball[0].isOnPaddle() && playerNum == 1) {
			...
			for (int b = 0; b < n_balls; b++) {
				if (ball[b].isOnPaddle()) {
					ball[b].setSpeedX(2.5f * ((ball[b].getX() - paddle.getPos()) / (paddle.getSize()) * 2));
					ball[b].setSpeedY(-(float) Math.sqrt(6.25 - ball[b].getSpeedX() * ball[b].getSpeedX()));
					ball[b].setOnPaddle(false);
				}
			}
		}
				...
				if (ball[b].isOnPaddle()) {
					...
					case 2:
						ball[b].setSpeedY(((ball[b].getY() - (border_bottom - right_paddle.getPos()))
								/ (right_paddle.getSize()) * 2));
						ball[b].setSpeedX(-(float) Math.sqrt(6.25 - ball[b].getSpeedY() * ball[b].getSpeedY()));

						break;

					case 3:
						ball[b].setSpeedX(2.5f * ((ball[b].getX() - (border_right - comp_paddle.getPos()))
								/ (comp_paddle.getSize()) * 2));
						ball[b].setSpeedY((float) Math.sqrt(6.25 - ball[b].getSpeedX() * ball[b].getSpeedX()));
					...
					case 4:
						ball[b].setSpeedY(
								2.5f * ((ball[b].getY() - left_paddle.getPos()) / (left_paddle.getSize()) * 2));
						ball[b].setSpeedX((float) Math.sqrt(6.25 - ball[b].getSpeedY() * ball[b].getSpeedY()));
	...
	public void keyPressed(KeyEvent arg0) {
		if (clicked_ip_box) {
			...
			case KeyEvent.VK_BACK_SPACE:
				ip_text = (ip_text.length() == 0) ? "" : ip_text.substring(0, ip_text.length() - 1);
				break;
			default:
